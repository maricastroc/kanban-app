...
interface TaskContextData {
  ...
  moveTaskToColumn: (task: TaskProps, targetColumn: string, previousColumn: string) => void;
}
...
export function TaskContextProvider({ children }: TaskContextProviderProps) {
  const { activeBoard, allBoards, handleSetActiveBoard, updateBoards } = useBoardsContext();

  function findActiveBoardIndex() {
    return allBoards.findIndex((board) => board.name === activeBoard?.name);
  }

  function moveTaskToColumn(task: TaskProps, targetColumnName: string, previousColumnName: string) {
    ...
    const sourceColumn = boardCopy?.columns?.find((column) => column.name === previousColumnName);
    if (!sourceColumn) return;

    const taskIndex = sourceColumn.tasks.findIndex((t) => t.title === task.title);
    ...
    const targetColumn = boardCopy?.columns?.find((column) => column.name === targetColumnName);
    if (!targetColumn) return;

    const isTaskDuplicate = targetColumn.tasks.some((t) => t.title === taskToMove.title);
    if (isTaskDuplicate) {
  ...
  function editTask(updatedTask: TaskProps, originalTask: TaskProps) {
    ...
    const boardIndex = boardsCopy.findIndex((board) => board.name === activeBoard?.name);

    if (boardIndex !== -1) {
      const boardCopy = { ...boardsCopy[boardIndex] };

      const columnIndex = boardCopy.columns.findIndex((column: BoardColumnProps) =>
        column.tasks.some((task: TaskProps) => task.title === originalTask.title)
      );

      if (columnIndex !== -1) {
        const column = boardCopy.columns[columnIndex];
        const isDuplicateTask = column.tasks.some(
          (task: TaskProps) => task.title === updatedTask.title && updatedTask.title !== originalTask.title
        );
        ...
        const taskToEdit = column.tasks.find((task: TaskProps) => task.title === originalTask.title);
        if (taskToEdit) {
  ...
  function addTaskToColumn(task: TaskProps, columnName: string | undefined) {
    ...
    const isDuplicateTask = column.tasks.some(
      (existingTask) => existingTask.title.toLowerCase() === task.title.toLowerCase()
    );
  ...
  function updateBoardColumns(updatedColumns: BoardColumnProps[]) {
    ...
    const columnNames = updatedColumns.map((column) => column.name.toLowerCase());
    const hasDuplicateColumns = new Set(columnNames).size !== updatedColumns.length;
    ...
    updatedColumns.forEach((columnToUpdate) => {
      const columnIndex = boardCopy.columns.findIndex((column) => column.name === columnToUpdate.name);
    ...
    boardCopy.columns = boardCopy.columns.filter((column) =>
      updatedColumns.some((columnToUpdate) => columnToUpdate.name === column.name)
    );
